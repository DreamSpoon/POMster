...
PARALLAX_HEIGHTMAP_MAT_NG_NAME = "ParallaxUV.POMSTER.MatNG"
...
def create_prereq_util_node_group(node_group_name, node_tree_type):
    if node_tree_type == 'ShaderNodeTree':
        if node_group_name == PARALLAX_HEIGHTMAP_MAT_NG_NAME:
...
def create_mat_ng_parallax_uv():
    ...
    new_nodes = {}
    new_node_group = bpy.data.node_groups.new(name=PARALLAX_HEIGHTMAP_MAT_NG_NAME, type='ShaderNodeTree')
    ...
    new_node_group.inputs.new(type='NodeSocketVector', name="Tangent UVMap")
    ...
    tree_links.new(new_nodes["Vector Math.006"].outputs[0], new_nodes["Group Output"].inputs[0])
    tree_links.new(new_nodes["Geometry"].outputs[1], new_nodes["Vector Math"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Vector Math.004"].inputs[0])
    ...
    tree_links.new(new_nodes["Vector Math.009"].outputs[1], new_nodes["Combine XYZ.002"].inputs[0])
    tree_links.new(new_nodes["Vector Math.007"].outputs[1], new_nodes["Combine XYZ.002"].inputs[1])
    tree_links.new(new_nodes["Vector Math.008"].outputs[1], new_nodes["Combine XYZ.002"].inputs[2])
    tree_links.new(new_nodes["Vector Math"].outputs[0], new_nodes["Vector Math.007"].inputs[0])
    tree_links.new(new_nodes["Geometry"].outputs[1], new_nodes["Vector Math.008"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.002"].outputs[0], new_nodes["Vector Math.005"].inputs[1])
    tree_links.new(new_nodes["Math"].outputs[0], new_nodes["Vector Math.004"].inputs[3])
    tree_links.new(new_nodes["Vector Math.008"].outputs[1], new_nodes["Math"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[2], new_nodes["Vector Math.010"].inputs[3])
    tree_links.new(new_nodes["Group Input"].outputs[3], new_nodes["Vector Math.009"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[3], new_nodes["Vector Math"].inputs[1])
    ...
    tree_links.new(new_nodes["Geometry"].outputs[4], new_nodes["Vector Math.008"].inputs[1])
    tree_links.new(new_nodes["Geometry"].outputs[4], new_nodes["Vector Math.007"].inputs[1])
    tree_links.new(new_nodes["Geometry"].outputs[4], new_nodes["Vector Math.009"].inputs[1])
...
def create_mat_ng_jitter3():
    ...
    new_node_group.outputs.new(type='NodeSocketFloat', name="Top")
    new_node_group.outputs.new(type='NodeSocketFloat', name="Middle")
    new_node_group.outputs.new(type='NodeSocketFloat', name="Bottom")
    tree_nodes = new_node_group.nodes
    # delete all nodes
    tree_nodes.clear()
...
def create_heightmap_apply_nodes(tree_nodes, tree_links, user_heightmap_node, user_input_index, user_output_index):
    ...
    node_loc_offset = (user_heightmap_node.location[0] + 960, user_heightmap_node.location[1] - 500)

    # create a Texture Coordinate input node if no input UV coordinates are available
    if len(user_heightmap_node.inputs[user_input_index].links) < 1:
        node = tree_nodes.new(type="ShaderNodeTexCoord")
        node.location = (-1680, 160)
...
#    node.uv_map = "UVMap"
    ...
    node = tree_nodes.new(type="ShaderNodeMath")
    node.location = (-1140, 0)
    node.operation = "ADD"
    node.use_clamp = False
    new_nodes["Math.015"] = node

    node = tree_nodes.new(type="ShaderNodeMath")
    node.location = (-1140, 320)
    node.operation = "DIVIDE"
    node.use_clamp = False
    new_nodes["Math.002"] = node
    ...
    new_nodes["Group.023"] = node

    node = tree_nodes.new(type="ShaderNodeGroup")
    node.location = (-1500, -600)
    node.node_tree = bpy.data.node_groups.get(PARALLAX_HEIGHTMAP_MAT_NG_NAME)
    new_nodes["Group.007"] = node

    node = tree_nodes.new(type="ShaderNodeGroup")
    node.location = (-1500, -170)
    node.node_tree = bpy.data.node_groups.get(PARALLAX_HEIGHTMAP_MAT_NG_NAME)
    ...
    # create links
    tree_links.new(input_uv_link_socket, new_nodes["Group.001"].inputs[0])
    tree_links.new(new_nodes["Group.002"].outputs[0], new_nodes["Group.001"].inputs[2])
    tree_links.new(new_nodes["Group.002"].outputs[1], new_nodes["Group.006"].inputs[2])
    tree_links.new(new_nodes["Group.002"].outputs[2], new_nodes["Group.007"].inputs[2])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Group.001"].inputs[1])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Group.006"].inputs[1])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Group.007"].inputs[1])
    tree_links.new(input_uv_link_socket, new_nodes["Group.006"].inputs[0])
    tree_links.new(input_uv_link_socket, new_nodes["Group.007"].inputs[0])
    tree_links.new(new_nodes["Group.001"].outputs[0], new_nodes["Group.003"].inputs[0])
    tree_links.new(new_nodes["UserGroup.Original"].outputs[0], new_nodes["Group.002"].inputs[0])
    tree_links.new(new_nodes["Group.007"].outputs[0], new_nodes["Group.010"].inputs[0])
    tree_links.new(new_nodes["Group.006"].outputs[0], new_nodes["Group.005"].inputs[0])
    tree_links.new(input_uv_link_socket, new_nodes["Group.009"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Group.009"].inputs[1])
    tree_links.new(input_uv_link_socket, new_nodes["Group.012"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Group.012"].inputs[1])
    tree_links.new(new_nodes["Group.009"].outputs[0], new_nodes["Group.013"].inputs[0])
    tree_links.new(new_nodes["Tangent"].outputs[0], new_nodes["Group.001"].inputs[3])
    tree_links.new(new_nodes["Tangent"].outputs[0], new_nodes["Group.009"].inputs[3])
    tree_links.new(new_nodes["Tangent"].outputs[0], new_nodes["Group.012"].inputs[3])
    tree_links.new(new_nodes["Tangent"].outputs[0], new_nodes["Group.007"].inputs[3])
    tree_links.new(new_nodes["Tangent"].outputs[0], new_nodes["Group.006"].inputs[3])
    tree_links.new(new_nodes["Group.003"].outputs[0], new_nodes["Group.017"].inputs[0])
    tree_links.new(new_nodes["Group.005"].outputs[0], new_nodes["Group.017"].inputs[2])
    tree_links.new(new_nodes["Group.010"].outputs[0], new_nodes["Group.017"].inputs[4])
    tree_links.new(new_nodes["Value.001"].outputs[0], new_nodes["Group.017"].inputs[6])
    tree_links.new(new_nodes["Group.023"].outputs[1], new_nodes["Math.015"].inputs[1])
    tree_links.new(new_nodes["Group.013"].outputs[0], new_nodes["Group.023"].inputs[0])
    tree_links.new(new_nodes["Value.001"].outputs[0], new_nodes["Group.023"].inputs[2])
    tree_links.new(new_nodes["Group.002"].outputs[2], new_nodes["Group.017"].inputs[5])
    tree_links.new(new_nodes["Group.002"].outputs[1], new_nodes["Group.017"].inputs[3])
    tree_links.new(new_nodes["Group.002"].outputs[0], new_nodes["Group.017"].inputs[1])
    tree_links.new(new_nodes["Group.017"].outputs[1], new_nodes["Math.015"].inputs[0])
    tree_links.new(input_uv_link_socket, new_nodes["UserGroup.Original"].inputs[0])
    tree_links.new(new_nodes["Group.017"].outputs[1], new_nodes["Math"].inputs[1])
    tree_links.new(new_nodes["Group.017"].outputs[0], new_nodes["Math"].inputs[0])
    tree_links.new(new_nodes["Math"].outputs[0], new_nodes["Group.009"].inputs[2])
    tree_links.new(new_nodes["Math"].outputs[0], new_nodes["Group.023"].inputs[1])
    tree_links.new(new_nodes["Group.023"].outputs[0], new_nodes["Math.001"].inputs[1])
    tree_links.new(new_nodes["Group.017"].outputs[0], new_nodes["Math.001"].inputs[0])
    ...
    tree_links.new(new_nodes["Math.002"].outputs[0], new_nodes["Group.012"].inputs[2])

    # fix output links to re-create original user node's output links, but use calculated height
    relink_final_user_node(tree_links, final_user_node, new_nodes["Group.012"].outputs[0], output_to_sockets,
                           user_input_index)
...
def create_heightmap_node_setup(node_tree, override_create, user_heightmap_node, user_input_index, user_output_index):
    ensure_node_groups(override_create,
                       [PARALLAX_HEIGHTMAP_MAT_NG_NAME,
                        JITTER3_MAT_NG_NAME,
...
class POMSTER_CreateHeightmapParallaxTexture(bpy.types.Operator):
    bl_description = "Using selected node as a basis, add nodes to create a Heightmap Parallax Effect. Selected node " \
        "must have at least one vector input and at least one value output"
    bl_idname = "pomster.create_heightmap_parallax_texture"
    bl_label = "Heightmap Nodes"
    ...
    def execute(self, context):
        ...
        if len(sel_nodes) != 1:
            self.report({'ERROR'}, "Unable to create Parallax Occlusion Mapping nodes because more than one " +
                        "Shader Node is selected in the editor.")
            return {'CANCELLED'}
        ...
        if len(user_node.inputs) < scn.POMSTER_UV_InputIndex:
            self.report({'ERROR'}, "Unable to create Parallax Occlusion Mapping nodes because selected heightmap " +
                        "node does not have enough inputs to get input number " + str(scn.POMSTER_UV_InputIndex) + " .")
            return {'CANCELLED'}
        # check user selected node to ensure minimum amount of outputs
        if len(user_node.inputs) < scn.POMSTER_HeightOutputIndex:
            self.report({'ERROR'}, "Unable to create Parallax Occlusion Mapping nodes because selected heightmap " +
                        "node does not have enough outputs to get output number " +
        ...
        if not hasattr(user_node.inputs[scn.POMSTER_UV_InputIndex-1], 'default_value') or \
            ...
            self.report({'ERROR'}, "Unable to create Parallax Occlusion Mapping nodes because selected heightmap " +
                        "node's input number " + str(scn.POMSTER_UV_InputIndex) + " is not a Vector type.")
            return {'CANCELLED'}
        # create the heightmap node setup in the current editor tree, with user_node as heightmap node
        create_heightmap_node_setup(context.space_data.edit_tree, scn.POMSTER_NodesOverrideCreate, user_node,
